...
namespace SampleSite
{
    public partial class ControlForm : Form
    {
        ...
        public ControlForm()
        {
            ...
            GetHandler.BeforeGet += new HttpEventHandler(GetHandler_BeforeGet);
        }

        void GetHandler_BeforeGet(OpenNETCF.Web.IHttpHandler source, OpenNETCF.Web.HttpContext context)
        {
            var s = string.Format("{0} received at {1} - handled by {2}", context.Request.RequestType, context.Request.Path, source.GetType().Name);
        ...
        private void UpdateIP()
        {
            ...
            {
                this.padarnIP.Text = "Padarn IP: {All Unassigned}";
            }
            else
            {
                this.padarnIP.Text = "Padarn IP:" + m_ws.Configuration.LocalIP;
            }

            deviceIP.Text = "Device IP: " + Dns.GetHostEntry(Dns.GetHostName()).AddressList[0].ToString();
        }
